...
- **시뮬레이터 실증 검증 완료** — 임계값을 임시로 초 단위(15/30초, 이후 60/120초)로 낮춘 테스트 빌드로 3가지 확인: (1) Play가 토큰 대신 배고픔을 깎고 토큰은 그대로 유지됨 (2) 행복 4/4를 유지한 채 배고픔만 0이 되어도 sick 전환됨("so hungry...") (3) 성장은 시간에 비례해 연속 누적되고(예: 60초 중 48초 경과 시 80%), 배고픔이 50 밑으로 내려가면 임계값 시간이 지나도 멈춤. 검증 후 임계값은 원래 값(3일/10일)으로 원복
- **`monkeydo` 실행 시 뜨는 "Failed invoking symbol" 크래시 로그는 오탐** — 매번 `onStart():33`(`GamigotchiStats.tick()` 호출)을 가리키며 뜨지만, debug.xml 심볼 맵 대조 결과 실제로 그 줄을 정확히 가리킴(예전에 추측했던 "낡은 로그 재생"은 아님). 그런데도 스크린샷으로 본 실제 화면은 항상 정상 동작 — monkeydo가 앱을 처음 설치/실행할 때 한 번 실패하고 시뮬레이터가 자동으로 재시작해서 두 번째 실행이 성공하는 것으로 추정되나, 근본 원인은 아직 미확인. **결론: 이 로그만 보고 크래시로 판단하면 안 되고, 반드시 스크린샷으로 실제 화면을 확인해야 함**
- **PowerShell `PrintWindow`(PW_RENDERFULLCONTENT)로 시뮬레이터 스크린샷 자동 캡처하는 스크립트 작성** — 창이 포커스/최상단이 아니어도 캡처 가능(백그라운드 상태에서도 안정 동작), WSL에서 `powershell.exe -File`로 호출. `Get-Process simulator`의 `MainWindowHandle` + `GetWindowRect`로 창 크기 얻어서 캡처 후 PNG 저장
- **Play 배고픔 소모량 10→5로 완화** — 실기기 테스트 중 "놀아주는데 하트(배고픔)가 깎이는 게 생명력이 줄어드는 것처럼 느껴진다"는 피드백. 메커니즘(논다→배고파진다→밥이 필요해진다→달리기로 이어진다) 자체는 유지, 체감 강도만 완화
- **후속 논의 (문서화만, 구현은 보류)**: Play 시 텍스트 말풍선 대신 콩콩 뛰기+파티클 애니메이션 필요성(REQUIREMENTS.md "놀아주는 장면" 섹션에 추가), 메인 화면 하트/별 숫자 게이지를 없애고 표정+말풍선으로만 상태를 암시하는 방향 제안(원조 다마고치처럼 수치를 노출하지 않는 쪽 — REQUIREMENTS.md "방향 D" 섹션에 추가, 표정 아트 완료가 선행 조건)
